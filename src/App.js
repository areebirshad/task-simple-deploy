...
import { addTodo, deleteTodo, getAllTodos } from './services/services';
...
function App() {
  ...
  const onAddTodo =()=>{
   if(todo !== '' && order !== ''){
     ...
     .then((success)=>{
       setAllData((prev)=>[...prev,{title:todo,order,completed:false}])
       setError(false)
       setTodo('');
       setOrder('')
      }).catch((error)=>{
...
<br />
  ...
  {index+1}
          ...
          <i onClick={()=>onDeleteTodo(item.id)} className='fas fa-pen' ></i> 
            </td>
          </tr>
        ))
      }
